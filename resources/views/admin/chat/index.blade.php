...
@section('page-title')
    <title>Admin - Chat Application</title>
@endsection
            ...
            <div class="chat-system">
                ...
                <div class="user-list-box">
                    ...
                    <div class="people">
                        ...
                        @foreach($users as $user)
                            <div id="person_{{$user->id}}" class="person @if($loop->last) borde-none @endif" 
                                data-url = {{ route('admin.chat.chat', $user) }}
                                data-chat="person{{ $user->id }}">
                                <div class="user-info">
                                    <div class="f-head position-relative">
                                        <img src="{{asset($user->profile->getImage())}}" alt="avatar">
                                        <div class="loader dual-loader mx-auto position-absolute d-none" style="top: -7px; left: calc(50% - 24px);"></div>
                                    </div>
                                    <div class="f-body">
                                        <div class="meta-info">
                                            @php
                                                $unreads = auth()->user()->unreadNotifications()
                                                    ...
                                                    ->get();
                                            @endphp
                                            <span class="user-name" data-name="{{ $user->firstname }} {{ $user->lastname }}">{{ $user->firstname }} {{ $user->lastname }}</span>
                                            @if($unreads->count() > 0)
                                                <span class="badge badge-danger toggle-message">
                                                    {{ $unreads->count() }}
                                                </span>
                                                <span class="user-meta-time toggle-message">{{ $unreads->first()->created_at->diffForHumans() }}</span>
                                            @endif
                                        </div>
                                            @if($unreads->count() > 0)
                                                <span class="preview toggle-message">{{ $unreads->first()->data['message'] }}</span>
                                            @endif
                                    </div>
                                </div>
                            </div>
                ...
                <div class="chat-box">
                    ...
                    <div class="overlay-phone-call">
                        <div class="">
                            ...
                            <div class="calling-user-img">
                                <div class="">
                                    <img src="{{asset('assets/img/90x90.jpg')}}" alt="dynamic-image">
                                </div>
                    ...
                    <div class="overlay-video-call">
                        <img src="{{asset('assets/img/175x115.jpg')}}" class="video-caller" alt="video-chat">
                        <div class="">
                            <div class="calling-user-info">
                                <div class="d-flex">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-left-circle go-back-chat"><circle cx="12" cy="12" r="10"></circle><polyline points="12 8 8 12 12 16"></polyline><line x1="16" y1="12" x2="8" y2="12"></line></svg>
